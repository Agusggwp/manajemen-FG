                ...
                <CardContent className="p-3.5 sm:p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
                  <div className="space-y-1.5 flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
                      <span className="font-bold text-slate-900 text-sm sm:text-base mr-1">{sch.customer?.name}</span>
                      <Badge variant="outline" className="text-[10px] bg-white whitespace-nowrap py-0 px-2 h-5">{sch.photo_package?.name}</Badge>
                    ...
                    <div className="text-xs text-slate-600 flex flex-wrap items-center gap-x-3 gap-y-1">
                      <span className="flex items-center gap-1"><Calendar className="h-3.5 w-3.5 text-slate-400 shrink-0" /> {formatDate(sch.date)}</span>
                      <span className="flex items-center gap-1"><Clock className="h-3.5 w-3.5 text-slate-400 shrink-0" /> {sch.start_time?.substring(0, 5)} - {sch.end_time?.substring(0, 5)}</span>
                    </div>
                    ...
                    {sch.location_address && (
                      <p className="text-xs text-slate-500 pl-4.5 truncate">{sch.location_address}</p>
                    )}
                  </div>
